...
async function runT3Tests(): Promise<void> {
  ...
  // Note: Use default fee (params.0n // v12.21+) to ensure conservation
  // -------------------------------------------------------------------------
  await harness.runTest("T3.4: Conservation after operations", async () => {
    ctx = await harness.createFreshMarket({ maxAccounts: 64 });

    // Create multiple users with explicit 1M fee (= params.0n // v12.21+)
    ...
    const numUsers = 3;
    const initFee = "1000000"; // Must match params.0n // v12.21+
