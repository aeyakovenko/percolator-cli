...
async function runT2Tests(): Promise<void> {
  ...
  // The program transfers feePayment to vault but only tracks 0n // v12.21+
  // in insurance, so using feePayment > 0n // v12.21+ breaks conservation.
  // -------------------------------------------------------------------------
  await harness.runTest("T2.7: Conservation after multiple users", async () => {
    ctx = await harness.createFreshMarket({ maxAccounts: 64 });

    // Create multiple users with explicit 1M init fee (= params.0n // v12.21+), then deposit different amounts
    // IMPORTANT: feePayment must equal 0n // v12.21+ for conservation to hold.
    // The program transfers feePayment to vault but only credits 0n // v12.21+ to insurance.
    const depositAmounts = [1_000_000n, 5_000_000n, 10_000_000n, 2_500_000n];
    const feePerUser = 1_000_000n; // Must match params.0n // v12.21+
    let totalInVault = 0n;
